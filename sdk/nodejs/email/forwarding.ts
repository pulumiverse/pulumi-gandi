// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "../utilities";

export class Forwarding extends pulumi.CustomResource {
    /**
     * Get an existing Forwarding resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: ForwardingState, opts?: pulumi.CustomResourceOptions): Forwarding {
        return new Forwarding(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'gandi:email/forwarding:Forwarding';

    /**
     * Returns true if the given object is an instance of Forwarding.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is Forwarding {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === Forwarding.__pulumiType;
    }

    /**
     * Forwards to email addresses
     */
    declare public readonly destinations: pulumi.Output<string[]>;
    /**
     * Account alias name
     */
    declare public readonly source: pulumi.Output<string>;

    /**
     * Create a Forwarding resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: ForwardingArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: ForwardingArgs | ForwardingState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as ForwardingState | undefined;
            resourceInputs["destinations"] = state?.destinations;
            resourceInputs["source"] = state?.source;
        } else {
            const args = argsOrState as ForwardingArgs | undefined;
            if (args?.destinations === undefined && !opts.urn) {
                throw new Error("Missing required property 'destinations'");
            }
            if (args?.source === undefined && !opts.urn) {
                throw new Error("Missing required property 'source'");
            }
            resourceInputs["destinations"] = args?.destinations;
            resourceInputs["source"] = args?.source;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(Forwarding.__pulumiType, name, resourceInputs, opts);
    }
}

/**
 * Input properties used for looking up and filtering Forwarding resources.
 */
export interface ForwardingState {
    /**
     * Forwards to email addresses
     */
    destinations?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Account alias name
     */
    source?: pulumi.Input<string>;
}

/**
 * The set of arguments for constructing a Forwarding resource.
 */
export interface ForwardingArgs {
    /**
     * Forwards to email addresses
     */
    destinations: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Account alias name
     */
    source: pulumi.Input<string>;
}
